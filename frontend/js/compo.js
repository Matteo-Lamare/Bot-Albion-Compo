...
function indexerCatalogue(donnees) {
  ...
  donnees.objets.forEach((objet) => {
    (objetsParSlot[objet.slot] ||= []).push({ ...objet, groupe: donnees.categories[objet.categorie_id].nom });
  });
...
async function demarrer() {
  ...
  if (compoId) await chargerCompo(compoId); else { ["bouton-discord", "bouton-apercu", "bouton-dupliquer"].forEach((id) => { document.getElementById(id).disabled = true; }); ajouterLigne(); majCompteur(); }
}
...
async function chargerCompo(id) {
  try {
    ...
    document.getElementById("nom").value = compo.nom; document.getElementById("type_contenu").value = compo.type_contenu; document.getElementById("taille_groupe").value = compo.taille_groupe; document.getElementById("statut").value = compo.statut; document.getElementById("notes").value = compo.notes || "";
...
function ajouterLigne(donnees = null, replie = false) {
  ...
  const entete = document.createElement("div"); entete.className = "ligne-entete"; entete.innerHTML = `<span class="numero"></span><span class="resume"></span><button type="button" class="mini" data-action="monter" title="Monter">↑</button><button type="button" class="mini" data-action="descendre" title="Descendre">↓</button><button type="button" class="mini" data-action="copier" title="Dupliquer la ligne">⧉</button><button type="button" class="mini danger" data-action="supprimer" title="Supprimer">✕</button>`;
  ...
  cat.slots.forEach((slot) => corps.appendChild(creerSlot(carte, slot))); carte.appendChild(entete); carte.appendChild(corps); conteneur.appendChild(carte);
...
function lireLigne(carte) { const donnees = { role_ou_joueur: carte.champRole.value.trim() }; Object.entries(carte.selecteurs).forEach(([champ, selecteur]) => { donnees[champ] = selecteur.valeur ?? null; }); return donnees; }
function majResume(carte) { const donnees = lireLigne(carte); const nom = (identifiant) => (identifiant ? cat.objets[identifiant]?.nom : null); const morceaux = [donnees.arme_id, donnees.casque_id, donnees.torse_id, donnees.bottes_id].map(nom).filter(Boolean).join(" · "); carte.querySelector(".resume").textContent = `${donnees.role_ou_joueur || libelleParDefaut(carte)}${morceaux ? " — " + morceaux : ""}`; }
function libelleParDefaut(carte) { const rang = [...document.querySelectorAll(".ligne-compo")].indexOf(carte); return `Build #${rang + 1}`; }
function majCompteur() { const cartes = document.querySelectorAll(".ligne-compo"); cartes.forEach((carte, index) => { carte.querySelector(".numero").textContent = `#${index + 1}`; }); const taille = parseInt(document.getElementById("taille_groupe").value, 10); const compteur = document.getElementById("compteur-lignes"); compteur.textContent = `${cartes.length} ligne(s)`; compteur.title = cartes.length === taille ? "" : "Différent de la taille de groupe annoncée"; compteur.style.color = cartes.length === taille ? "" : "var(--or)"; }
function basculerTout(replie) { document.querySelectorAll(".ligne-compo").forEach((carte) => carte.classList.toggle("replie", replie)); }
function validerFormulaire() { const erreurs = []; document.querySelectorAll(".erreur").forEach((champ) => champ.classList.remove("erreur")); document.querySelectorAll(".ligne-compo").forEach((carte, index) => { const numero = index + 1; const signaler = (element, libelle, message) => { element.classList.add("erreur"); erreurs.push({ champ: `Ligne ${numero} · ${libelle}`, message }); carte.classList.remove("replie"); }; cat.slots.forEach((slot) => { const selecteurObjet = carte.selecteurs[slot.champ]; if (!selecteurObjet.valeur) { if (slot.obligatoire) signaler(selecteurObjet, slot.libelle, "objet obligatoire"); return; } slot.champs.forEach((definition) => { const selecteur = carte.selecteurs[definition.champ]; if (definition.obligatoire && !selecteur.parentElement.hidden && !selecteur.valeur) signaler(selecteur, `${slot.libelle} · ${definition.libelle}`, "choix obligatoire"); }); }); const donnees = lireLigne(carte); if (donnees.torse_passif_1_id && donnees.torse_passif_1_id === donnees.torse_passif_2_id) signaler(carte.selecteurs.torse_passif_2_id, "Torse · Passif 2", "les deux passifs doivent être différents"); if (donnees.offhand_id && cat.objets[donnees.arme_id]?.deux_mains) signaler(carte.selecteurs.offhand_id, "Off-hand", "l'arme se tient à deux mains : pas d'off-hand"); }); return erreurs; }
function lireFormulaire() { return { nom: document.getElementById("nom").value.trim(), type_contenu: document.getElementById("type_contenu").value, taille_groupe: parseInt(document.getElementById("taille_groupe").value, 10), statut: document.getElementById("statut").value, notes: document.getElementById("notes").value.trim() || null, lignes: [...document.querySelectorAll(".ligne-compo")].map((carte, index) => ({ ...lireLigne(carte), ordre: index })) }; }
async function enregistrer(evenement) { evenement.preventDefault(); cacherMessage("message"); const erreurs = validerFormulaire(); if (erreurs.length) { afficherMessage("message", "Corrigez les champs suivants :", "erreur", erreurs); return; } const donnees = lireFormulaire(), bouton = evenement.target.querySelector("button[type=submit]"); bouton.disabled = true; try { const compo = compoId ? await api.modifierCompo(compoId, donnees) : await api.creerCompo(donnees); if (!compoId) { location.href = `/compo?id=${compo.id}`; return; } const cartes = [...document.querySelectorAll(".ligne-compo")]; compo.lignes.forEach((ligne, index) => { if (cartes[index]) cartes[index].ligneId = ligne.id; }); afficherMessage("message", "Compo enregistrée.", "succes"); document.getElementById("info-compo").textContent = `Auteur : ${compo.auteur_pseudo} · modifiée le ${formaterDate(compo.date_modification)}`; } catch (erreur) { afficherMessage("message", erreur.message, "erreur", traduireErreurs(erreur.erreurs)); } finally { bouton.disabled = false; } }
function traduireErreurs(erreurs) { return (erreurs || []).map((erreur) => { const morceaux = (erreur.champ || "").split("."); if (morceaux[0] === "lignes" && morceaux.length >= 2) { const champ = (morceaux.slice(2).join(".") || "ligne").replace(/_id$/, ""); return { champ: `Ligne ${parseInt(morceaux[1], 10) + 1} · ${champ}`, message: erreur.message }; } return erreur; }); }
async function apercu() { cacherMessage("message"); try { const resultat = await api.apercuDiscord(compoId), zone = document.getElementById("zone-apercu"), contenu = document.getElementById("contenu-apercu"); contenu.innerHTML = ""; const entete = document.createElement("div"); entete.className = "apercu entete-apercu"; entete.textContent = resultat.entete; contenu.appendChild(entete); resultat.apercu.forEach((build) => { const bloc = document.createElement("div"); bloc.className = "apercu"; if (resultat.images_disponibles) { const image = document.createElement("img"); image.className = "image-build"; image.loading = "lazy"; image.alt = build.titre; image.src = build.image; bloc.appendChild(image); } else { const secours = document.createElement("div"); secours.className = "titre-champ"; secours.textContent = `${build.titre} (image indisponible : Pillow n'est pas installé)`; bloc.appendChild(secours); } contenu.appendChild(bloc); }); zone.hidden = false; zone.scrollIntoView({ behavior: "smooth" }); } catch (erreur) { afficherMessage("message", erreur.message, "erreur", erreur.erreurs); } }
async function envoyerDiscord() { const webhook = prompt("Webhook Discord pour cet envoi (il ne sera pas enregistré) :"); if (!webhook) return; if (!confirm("Envoyer cette compo sur le salon Discord ?")) return; cacherMessage("message"); const bouton = document.getElementById("bouton-discord"); bouton.disabled = true; try { const resultat = await api.envoyerDiscord(compoId, webhook.trim()); afficherMessage("message", `Compo envoyée sur Discord : ${resultat.messages_envoyes} message(s), ${resultat.images_jointes} image(s) de build. Statut : ${resultat.statut}, le ${formaterDate(resultat.date_envoi)}.`, "succes"); document.getElementById("statut").value = resultat.statut; } catch (erreur) { afficherMessage("message", erreur.message, "erreur", erreur.erreurs); } finally { bouton.disabled = false; } }
async function importerTableur(evenement) { const fichier = evenement.target.files[0]; evenement.target.value = ""; if (!fichier) return; cacherMessage("message"); try { const resultat = await api.importerTableur(fichier); document.querySelectorAll(".ligne-compo").forEach((carte) => { if (!carte.selecteurs.arme_id.valeur) carte.remove(); }); resultat.lignes.forEach((ligne) => ajouterLigne(ligne, true)); majCompteur(); afficherMessage("message", `${resultat.lignes.length} build(s) importés depuis « ${fichier.name} ». Vérifiez-les puis enregistrez la compo.`, "succes", resultat.avertissements.map((texte) => ({ message: texte }))); } catch (erreur) { afficherMessage("message", erreur.message, "erreur", erreur.erreurs); } }
async function dupliquer() { try { const copie = await api.dupliquerCompo(compoId); location.href = `/compo?id=${copie.id}`; } catch (erreur) { afficherMessage("message", erreur.message, "erreur", erreur.erreurs); } }
