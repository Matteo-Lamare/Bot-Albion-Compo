...
async function appelApi(chemin, options = {}) {
  ...
  if (!reponse.ok) {
    ...
    throw new ErreurApi(typeof detail === "string" ? detail : "Une erreur est survenue.", reponse.status, erreurs);
  }
  return corps;
}

class ErreurApi extends Error {
  constructor(message, statut, erreurs) { super(message); this.statut = statut; this.erreurs = erreurs || []; }
}
...
const api = {
  ...
  creerMembre: (donnees) => appelApi("/api/membres", { method: "POST", body: JSON.stringify(donnees) }),
  modifierMembre: (id, donnees) => appelApi(`/api/membres/${id}`, { method: "PUT", body: JSON.stringify(donnees) }),
  supprimerMembre: (id) => appelApi(`/api/membres/${id}`, { method: "DELETE" }),
  compos: (filtres = {}) => {
    const params = new URLSearchParams();
    Object.entries(filtres).forEach(([cle, valeur]) => { if (valeur !== "" && valeur !== null && valeur !== undefined) params.append(cle, valeur); });
    const suffixe = params.toString() ? `?${params}` : "";
    return appelApi(`/api/compos${suffixe}`);
  },
  compo: (id) => appelApi(`/api/compos/${id}`),
  creerCompo: (donnees) => appelApi("/api/compos", { method: "POST", body: JSON.stringify(donnees) }),
  modifierCompo: (id, donnees) => appelApi(`/api/compos/${id}`, { method: "PUT", body: JSON.stringify(donnees) }),
  ...
  envoyerDiscord: (id, webhookUrl, typeSalon = "text") => appelApi(`/api/compos/${id}/envoyer-discord`, { method: "POST", body: JSON.stringify({ webhook_url: webhookUrl, type_salon: typeSalon }) }),
...
function afficherMessage(id, texte, categorie = "info", erreurs = []) {
  const bloc = document.getElementById(id); if (!bloc) return;
  bloc.className = `message visible ${categorie}`; bloc.innerHTML = ""; bloc.appendChild(document.createTextNode(texte));
  if (erreurs.length) { const liste = document.createElement("ul"); erreurs.forEach((erreur) => { const item = document.createElement("li"); item.textContent = erreur.champ ? `${erreur.champ} : ${erreur.message}` : erreur.message; liste.appendChild(item); }); bloc.appendChild(liste); }
  bloc.scrollIntoView({ behavior: "smooth", block: "nearest" });
}
function cacherMessage(id) { const bloc = document.getElementById(id); if (bloc) bloc.className = "message"; }
function formaterDate(valeur) { if (!valeur) return "—"; const date = new Date(valeur.endsWith("Z") || valeur.includes("+") ? valeur : `${valeur}Z`); return date.toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" }); }
function classeStatut(statut) { return { brouillon: "brouillon", "validée": "validee", "envoyée": "envoyee" }[statut] || ""; }
